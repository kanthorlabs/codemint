...
//
// Note: Task execution (dispatching to ACP worker) is handled by Executor.Execute()
// which directly uses the ACP Pipeline. This interface only defines the
// Accept/Revert contract for atomic undo (Story 1.9).
type CodingAgent interface {
	...
	// file-system changes made during task execution. If the agent returns an
